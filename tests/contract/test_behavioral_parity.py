...
from nl2sql_analytics.adapters._review_payload import (
    result_to_review,
)
...
def test_the_payload_that_reaches_the_wire_is_redacted_whichever_family_built_it() -> None:
    ...
    answer = AnalystAnswer(
        question="q",
        subject=f"revenue for NRIC {planted}",
        metric_id="revenue",
    ...
    payload = repr(
        result_to_review(answer, maker=sample_cases.ACTOR, tenant=sample_cases.TENANT).to_payload()
    )
...
def test_the_whole_pipeline_answers_on_local_and_fails_fast_on_onprem() -> None:
    ...
    answer = analyst_service(local).answer(
        sample_cases.CONDITIONAL_QUESTION, actor=sample_cases.ACTOR
    )
    assert answer.requires_human_review is True
    assert answer.citations, "an offline answer must still be cited"
    assert local.review_router.route(answer, maker=sample_cases.ACTOR)

    # The exit family refuses at its seams: no code edited, one profile line swapped. The
    # orchestrator itself fails CLOSED (an unreachable port becomes a refusal), so the fail-fast
    # is proven at the port the client rebinds, which is where the exit placeholder lives.
    onprem = build_container(local_settings(profile="onprem"))
    with pytest.raises(NotImplementedError):
        onprem.query_engine.execute(sample_cases.CANONICAL_QUERY)
