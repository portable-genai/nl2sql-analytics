...
def test_the_demo_data_is_obviously_fictional() -> None:
    """The demo drives natural-language questions, so the fake markers live in the question text
    and in the party constants, not in a free-form subject line the questions do not carry."""
    cases = (demo.ROUTINE_CASE, demo.ESCALATING_CASE, demo.PII_CASE)
    for case in cases:
        assert case.text.strip(), "a demo question carries no text to drive the service"
        assert case.tenant == demo.TENANT, "every demo question is scoped to the fictional tenant"
    blob = " ".join(case.text for case in cases) + " " + demo.ACTOR + " " + demo.TENANT
    # Obviously synthetic parties: reserved .example domains, never a real mailbox provider.
    ...
    # The one planted identifier is the documented synthetic national id, and the one host literal
    # is an RFC 3849 documentation address, so the redaction beat masks fake data, never a real
    # record. The IPv4 LAN peer the exposure tests use is an RFC 5737 literal for the same reason.
    assert demo.PLANTED_NRIC in blob, "the redaction beat needs its planted synthetic identifier"
    assert "2001:db8:" in blob, "the host literal must be an RFC 3849 documentation address"
