...
from nl2sql_analytics.adapters.gcp.identity import (
    _AUDIENCE_ENV,
...
from nl2sql_analytics.config import Settings, _read_settings_file
from nl2sql_analytics.ports.identity import (
    VERIFIED,
...
_API_MODULE = "nl2sql_analytics.api.app"
...
def _port_block(port: str, cls: str) -> list[str]:
    """One ``adapters:`` port entry binding ``local`` and ``gcp`` to the SAME SDK-free class.

    Every port except identity is rebound to its offline adapter under BOTH the ``local`` and
    ``gcp`` profiles, so these tests exercise the real gcp IDENTITY binding (``IapIdentityAdapter``)
    without needing Cloud Logging, a live H4 or a reachable console. The settings file must bind
    EVERY declared port in EVERY profile (``config._bindings_from`` refuses a partial block), so a
    port added to the service must be added here too, or the managed app fails to load.
    """
    return [
        f"  {port}:",
        *[f"    {p}: {_PKG}.adapters.local.{cls}" for p in ("local", "gcp")],
        f"    onprem: {_PKG}.adapters.onprem.{cls.replace('Local', 'OnPrem')}",
    ]
...
_REBOUND_SETTINGS = "\n".join(
    [
        ...
        *_port_block("review_router", "review_router:LocalReviewRouter"),
        *_port_block("certification", "certification:LocalCertificationAdapter"),
        *_port_block("dictionary", "dictionary:LocalDictionaryAdapter"),
        *_port_block("llm", "llm:LocalAnalystLlm"),
        *_port_block("query_engine", "query_engine:LocalQueryEngineAdapter"),
        *_port_block("guardrail", "guardrail:LocalGuardrailAdapter"),
        *_port_block("channel", "channel:LocalChannelAdapter"),
