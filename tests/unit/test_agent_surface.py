...
from nl2sql_analytics.agent import (
    SKILLS,
    TOOL_FUNCTIONS,
    agent_card_document,
    ask_question,
...
def test_the_card_document_is_json_safe_and_names_the_region() -> None:
    ...
    assert document["name"] == "nl2sql-analytics"
...
def test_the_agent_path_does_not_manufacture_a_review_for_a_certified_answer() -> None:
    result = ask_question(
        sample_cases.CERTIFIED_QUESTION.text,
        tenant=sample_cases.TENANT,
        settings=local_settings(),
    )
