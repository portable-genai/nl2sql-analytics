...
from nl2sql_analytics.adapters.local.audit import (
    LocalAuditAdapter,
)
...
_QUESTIONS = (
    sample_cases.CONDITIONAL_QUESTION,
    sample_cases.CERTIFIED_QUESTION,
    sample_cases.PII_QUESTION,
)

_ANCHOR_ENV = "NL2SQL_AUDIT_ANCHOR"
...
def _record_three(settings: Settings) -> LocalAuditAdapter:
    ...
    for question in _QUESTIONS:
        service.answer(question, actor=sample_cases.ACTOR)
    return audit
...
def test_an_append_after_truncation_cannot_relaunder_the_anchor(tmp_path: Path) -> None:
    ...
    with pytest.raises(AuditChainError):
        analyst_service(build_container(settings)).answer(
