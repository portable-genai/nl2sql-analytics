...
here instead of the profile's: :class:`DisabledGuardrail` (``NL2SQL_GUARDRAIL=off``) allows every
question with the reason ``"guardrail off"``, and :class:`DisabledReviewRouter`
(``NL2SQL_REVIEW_ROUTING=off``) submits nothing. Each satisfies its port, so no service needs a
``None`` branch, and the container logs the posture at startup.

**Recording wrapper.** Every caller that hands an answer to the router (the API route, the agent
...
failure is logged and absorbed here rather than failing an already-answered, already-audited
question, but it is never invisible: the caller reports ``failed`` and an empty reference,
...
from ..domain.models import AnalystAnswer, ScreenResult
...
class DisabledGuardrail:
    """GuardrailPort with the guardrail switched off: allows every question."""
    ...
    def screen(self, text: str) -> ScreenResult:
        return ScreenResult(allowed=True, reason="guardrail off")
