...
SKILLS: tuple[AgentSkill, ...] = (
    AgentSkill(
        id="ask_question",
        name="Governed analytical question",
        description=(
            "Answer a natural-language analytical question against the certified semantic layer, "
            "or refuse it. The model proposes an intent; pure code certifies it, gates it on the "
            "H4 certification of the backing dataset, composes and validates bounded read-only "
            "SQL, executes it, and cites every figure. An uncertified metric is refused, not "
            "guessed, and a conditionally certified answer routes to human review (rule R8)."
        ),
    ),
    AgentSkill(
...
_DESCRIPTION = " ".join(
    (
        "NL2SQL Semantic Analyst",
        "(H1).",
        "Governed NL-to-SQL over a certified semantic layer: deterministic",
        "composition, cited figures, redact-before-audit, refuse-the-uncertified,",
        "and every consequential answer routed to a human reviewer.",
    )
)
...
def _resolve_url(settings: Settings) -> str:
    ...
    return f"https://nl2sql-analytics.{settings.region}.internal.example/a2a"
