...
DEFAULT_ACTOR = "nl2sql-analytics-agent"
...
def ask_question(
    ...
    """Answer one governed analytical question, or refuse it, and route escalations for review.

    Screens the question, has the model propose an intent, certifies it against the semantic
    layer and the H4 certification gate, composes and validates bounded read-only SQL, executes
    it, and cites every figure. A conditionally certified backing dataset (or an empty result)
    escalates to the human-review console (rule R8). An uncertified metric is refused, not guessed.

    Args:
      question: The natural-language analytical question.
      actor: The verified identity this call is attributed to.
      tenant: The tenant partition the row-level policy scopes every query to.
...
TOOL_FUNCTIONS = (ask_question, verify_audit_trail)
