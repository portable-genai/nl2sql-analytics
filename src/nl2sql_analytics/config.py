...
One env var (``NL2SQL_PROFILE``) selects the adapter family for every
...
class ProfileChoice:
    ...
    The variable is ``NL2SQL_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("NL2SQL_PROFILE", "local")``: that fallback reads an
...
DEFAULT_BINDINGS: dict[str, dict[str, str]] = {
    ...
    "certification": {
        "local": f"{_PKG}.adapters.local.certification:LocalCertificationAdapter",
        "gcp": f"{_PKG}.adapters.gcp.certification:CloudCertificationAdapter",
        "onprem": f"{_PKG}.adapters.onprem.certification:OnPremCertificationAdapter",
    },
    "dictionary": {
        "local": f"{_PKG}.adapters.local.dictionary:LocalDictionaryAdapter",
        "gcp": f"{_PKG}.adapters.gcp.dictionary:CloudDictionaryAdapter",
        "onprem": f"{_PKG}.adapters.onprem.dictionary:OnPremDictionaryAdapter",
    },
    "llm": {
        "local": f"{_PKG}.adapters.local.llm:LocalAnalystLlm",
        "gcp": f"{_PKG}.adapters.gcp.llm:CloudAnalystLlm",
        "onprem": f"{_PKG}.adapters.onprem.llm:OnPremAnalystLlm",
    },
    "query_engine": {
        "local": f"{_PKG}.adapters.local.query_engine:LocalQueryEngineAdapter",
        "gcp": f"{_PKG}.adapters.gcp.query_engine:CloudQueryEngineAdapter",
        "onprem": f"{_PKG}.adapters.onprem.query_engine:OnPremQueryEngineAdapter",
    },
    "guardrail": {
        "local": f"{_PKG}.adapters.local.guardrail:LocalGuardrailAdapter",
        "gcp": f"{_PKG}.adapters.gcp.guardrail:CloudGuardrailAdapter",
        "onprem": f"{_PKG}.adapters.onprem.guardrail:OnPremGuardrailAdapter",
    },
    "channel": {
        "local": f"{_PKG}.adapters.local.channel:LocalChannelAdapter",
        "gcp": f"{_PKG}.adapters.gcp.channel:CloudChannelAdapter",
        "onprem": f"{_PKG}.adapters.onprem.channel:OnPremChannelAdapter",
    },
}
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``NL2SQL_SETTINGS``) that does not
...
#: unrecognised value refuses at boot. See the fleet's runtime-control contract.
GUARDRAIL_ENV = "NL2SQL_GUARDRAIL"
REVIEW_ROUTING_ENV = "NL2SQL_REVIEW_ROUTING"
#: The Model Armor template the managed guardrail screens through, read by the settings file.
MODEL_ARMOR_TEMPLATE_ENV = "NL2SQL_MODEL_ARMOR_TEMPLATE"
...
class Settings:
    ...
    project_id: str = ""
    #: The Model Armor template id (in ``project_id`` and ``region``) the managed guardrail
    #: screens every question through. Unused offline.
...
def _refuse_unconfigured_controls(settings: Settings) -> None:
    ...
    fail that request, and a Model Armor guardrail with no template or project would build a
    malformed URL on the first question, which the orchestrator reads as "screen unavailable"
    and refuses. Both are configuration errors, so both refuse here with the two ways out.
    """
    ...
    if settings.controls.guardrail and settings.adapters["guardrail"][settings.profile].endswith(
        ":CloudGuardrailAdapter"
    ):
        missing = [
            name
            for name, value in (
                (MODEL_ARMOR_TEMPLATE_ENV, settings.model_armor_template),
...
class Container:
    ...
    @cached_property
    def certification(self) -> CertificationPort:
        adapter = self._bind("certification")
        assert isinstance(adapter, CertificationPort)
        return adapter

    @cached_property
    def dictionary(self) -> DictionaryPort:
        adapter = self._bind("dictionary")
        assert isinstance(adapter, DictionaryPort)
        return adapter

    @cached_property
    def llm(self) -> AnalystLlmPort:
        adapter = self._bind("llm")
        assert isinstance(adapter, AnalystLlmPort)
        return adapter

    @cached_property
    def query_engine(self) -> QueryEnginePort:
        adapter = self._bind("query_engine")
        assert isinstance(adapter, QueryEnginePort)
        return adapter
    ...
    @cached_property
    def channel(self) -> ChannelPort:
        adapter = self._bind("channel")
