"""FastAPI application for NL2SQL Semantic Analyst (H1).
...
``NL2SQL_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``NL2SQL_S2S_TOKEN`` is wrong in the most dangerous direction:
...
_CORS_ORIGINS_ENV = "NL2SQL_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "NL2SQL_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "NL2SQL_S2S_AUDIENCE"
_API_HOST_ENV = "NL2SQL_API_HOST"
...
def _analyst() -> AnalystService:
    """The orchestrator, wired once to the cached container's ports and the certified layer."""
    return analyst_service(_container())
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``NL2SQL_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``NL2SQL_PROFILE`` must never be allowed to
    ...
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "NL2SQL_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_SERVICE_NAME = "nl2sql-analytics"
...
_APP_TITLE = "NL2SQL Semantic Analyst"
...
#
# Note what is NOT in this expression: NL2SQL_S2S_TOKEN. A service
...
@app.post("/v1/ask", response_model=AskResponse, tags=["artifacts"])
def ask(
    request: AskRequest,
    principal: Annotated[Principal, Depends(get_principal)],
) -> AskResponse:
    """Answer a governed question, or refuse it. The tenant and actor are the verified principal.
