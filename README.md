# NL2SQL Semantic Analyst (H1)

Governed NL-to-SQL analyst: certified semantic layer, deterministic SQL, cited answers.
...
nl2sql_analytics triage "Acme (FICTIONAL)" "urgent data breach"
```
...
One env var, `NL2SQL_PROFILE`, selects the adapter family:
...
`NL2SQL_ALLOW_INSECURE_DEMO=1` says otherwise. The guard reads the identity
BINDING to decide that, never a service credential: setting
`NL2SQL_S2S_TOKEN` closes the S2S routes and does not open anything else.
See `docs/runbook.md`.
...
**Name the profile.** `NL2SQL_PROFILE` has no default. Leaving it unset is
