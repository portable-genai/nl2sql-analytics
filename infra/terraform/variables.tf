...
variable "guardrail_enabled" {
  description = "Switch the Model Armor guardrail that screens every question (the service's _GUARDRAIL variable). A cheap runtime control: on in the reference, reversible, so it takes a default."
  type        = bool
  default     = true
}
