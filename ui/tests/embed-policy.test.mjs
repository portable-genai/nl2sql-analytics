...
test("a wildcard is refused, not quietly dropped from the list", () => {
  // Filtering the list, skipping a matched wildcard and honouring the surviving entries, makes
  // `https://a.bank.example *` resolve to the named origin alone, and the operator
  // who wrote the asterisk is never told it selected nothing. Refusing is the only answer that
  // reaches them, because the accepted half looks exactly like a correct configuration.
...
test("a PARTIAL wildcard is refused too, in both allowlists", () => {
  // The exact-token set was the whole check, so `https://*.example` was in no set, was accepted,
  // and was emitted verbatim. CSP honours a host-source wildcard, so every subdomain could frame
  // the console, including one obtained by subdomain takeover or serving user content; the same
  // shape in the CORS allowlist hands that subdomain a credentialed cross-origin call. Refusing
  // any asterisk turns away nothing a deployment could correctly hold, because a real origin
  // never contains the character.
  for (const wildcard of ["https://*.example", "*.example", "https://*", "*.bank.example:443"]) {
    assert.throws(
      () => frameAncestors({ UI_FRAME_ANCESTORS: wildcard }),
      ConfiguredEmptyError,
      "UI_FRAME_ANCESTORS=" + JSON.stringify(wildcard) + " was accepted",
    );
    assert.throws(
      () => tenantOrigins({ UI_TENANT_ORIGINS: wildcard }),
      ConfiguredEmptyError,
      "UI_TENANT_ORIGINS=" + JSON.stringify(wildcard) + " was accepted",
    );
    // A mixed list is the dangerous shape: one valid origin makes the value look configured, and
    // the wildcard beside it is the entry that actually widens the policy.
    const mixed = "https://a.bank.example " + wildcard;
    assert.throws(
      () => frameAncestors({ UI_FRAME_ANCESTORS: mixed }),
      ConfiguredEmptyError,
      "UI_FRAME_ANCESTORS=" + JSON.stringify(mixed) + " was accepted",
    );
    assert.throws(
      () => corsOriginFor("https://a.bank.example", { UI_TENANT_ORIGINS: mixed }),
      ConfiguredEmptyError,
      "UI_TENANT_ORIGINS=" + JSON.stringify(mixed) + " was accepted",
    );
    // The boot-time assertion has to refuse exactly what the per-request functions refuse, or a
    // deployment carrying the wildcard comes up and fails one request at a time instead.
    assert.throws(() => assertEmbedPolicyConfigured({ UI_FRAME_ANCESTORS: wildcard }), ConfiguredEmptyError);
    assert.throws(() => assertEmbedPolicyConfigured({ UI_TENANT_ORIGINS: wildcard }), ConfiguredEmptyError);
    // securityHeaders is the surface that would have emitted it, so it must refuse rather than
    // serve a frame-ancestors directive carrying a host-source wildcard.
    assert.throws(() => securityHeaders({ UI_FRAME_ANCESTORS: wildcard }), ConfiguredEmptyError);
  }
});

test("the literal null is refused as its own named token", () => {
  // Not a wildcard by spelling, a wildcard by behaviour: a sandboxed iframe presents a null
  // origin, so allowing it invites exactly the framing the directive exists to refuse.
  assert.throws(() => frameAncestors({ UI_FRAME_ANCESTORS: "null" }), ConfiguredEmptyError);
  assert.throws(() => tenantOrigins({ UI_TENANT_ORIGINS: "null" }), ConfiguredEmptyError);
  assert.throws(
    () => frameAncestors({ UI_FRAME_ANCESTORS: "https://a.bank.example null" }),
    ConfiguredEmptyError,
  );
  assert.equal(corsOriginFor("null", { UI_TENANT_ORIGINS: "https://a.bank.example" }), null);
});
