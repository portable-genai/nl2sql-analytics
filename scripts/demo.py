...
Every party, address and identifier here is obviously fictional: ``.example`` domains, an RFC 3849
documentation host literal, and a synthetic national id that exists only to prove redaction ran.
...
from nl2sql_analytics.assembly import (
    build_analyst,
)
...
from nl2sql_analytics.domain import (
    kernel,
    models,
)
...
SERVICE_NAME = "NL2SQL Semantic Analyst"
CATALOG_ID = "H1"
REPOSITORY = "nl2sql-analytics"
...
# A fully certified question (revenue rides orders_daily, which H4 certifies): answered, cited,
# not escalated.
ROUTINE_CASE = models.Question(text="What was total revenue by region?", tenant=TENANT)

# A conditionally certified question (active_customers rides customers, which H4 only
# conditionally certifies): answered with a caveat, escalated AND routed (rule R8).
ESCALATING_CASE = models.Question(text="How many active customers by segment?", tenant=TENANT)

# The same conditionally certified path, with personal data planted in the question text, so the
# redact-before-write beat has an identifier to prove it masks.
PII_CASE = models.Question(
    text=(
        "How many active customers by segment for NRIC "
        + PLANTED_NRIC
        + " at ops@gamma.example on host 2001:db8::7?"
    ),
    tenant=TENANT,
)
...
STEPS: tuple[Step, ...] = (
    ...
    Step(
        key="routine",
        label="A certified question: answered, cited, NOT escalated",
        narration=(
            "A question about a certified metric on a certified dataset. The model only proposes "
            "an intent; pure code certifies it, composes bounded read-only SQL and executes it. "
            "Every figure cites its metric definition, and nothing is routed for review."
        ),
    ),
    Step(
        key="escalation",
        label="A conditionally certified answer: escalated AND routed (rule R8)",
        narration=(
            "A metric whose backing dataset H4 only CONDITIONALLY certifies. The answer carries "
            "a caveat and is handed to the human-review console in the same call that produced "
            "it. Setting the flag is not the escalation; routing is, and the reference records it."
        ),
    ),
    Step(
        key="redaction",
        ...
        narration=(
            "The same path, with a national id planted in the question text. The identifier is "
            "masked before the model is called and before anything is written, so the immutable "
            "record never contains it. Redacting afterwards would be too late."
        ),
    ),
    Step(
        key="review_queue",
...
class DemoRun:
    ...
    def __init__(self, workdir: Path | None = None) -> None:
        ...
        self.container, self.service = build_analyst(self.settings)
    ...
    def _step_routine(self) -> Produced:
        return self._question_panels(ROUTINE_CASE, expect_routing=False)

    def _step_escalation(self) -> Produced:
        return self._question_panels(ESCALATING_CASE, expect_routing=True)

    def _step_redaction(self) -> Produced:
        panels, facts = self._question_panels(PII_CASE, expect_routing=True)
    ...
    def _question_panels(self, case: models.Question, *, expect_routing: bool) -> Produced:
        result = self.service.answer(case, actor=ACTOR)
        self.cases += 1
        review_ref = ""
        ...
        answered = "refused" if result.refused else result.metric_id
        decision = Panel(
            title="Answer: " + result.subject,
            rows=(
                Row("Question", case.text),
                Row("Metric", answered, "ok" if not result.refused else "warn"),
                Row("Certification", result.certification_status.value),
                Row("Composed SQL", result.sql or "(none: refused)"),
                Row("Summary", result.summary),
                Row("Requires human review", str(result.requires_human_review)),
                Row(
                    "Routed to review",
                    review_ref or "not routed (fully certified)",
                    "ok" if consistent else "bad",
                ),
                Row("Attributed to", ACTOR),
            ),
            note=(
                "The model only proposes an intent and narrates. Certification, composition and "
                "the escalate/refuse decision are pure code, and replayable."
            ),
        ...
        evidence = Panel(
            ...
            note="Every figure cites its metric definition and the dataset's certification status.",
        )
        facts = {
            "subject": result.subject,
            "metric_id": result.metric_id,
            "certification": result.certification_status.value,
...
#: A canonical answer and query the exit calls reuse for the review-router, channel and
#: query-engine seams. Every field is obviously fictional.
_CANONICAL_ANSWER = models.AnalystAnswer(
    question="How many active customers by segment?",
    subject="Active customers",
    metric_id="active_customers",
    certification_status=models.CertificationStatus.CONDITIONAL,
    ...
    result=models.QueryResult(columns=("segment", "active_customers"), rows=(("retail", "2"),)),
    requires_human_review=True,
    refused=False,
    citations=(
        kernel.Citation(source_id="metric:active_customers", title="Active customers", snippet=""),
    ),
)

_CANONICAL_QUERY = models.CompiledQuery(
    sql="SELECT region, SUM(amount) AS revenue FROM orders_daily WHERE tenant = :tenant "
    "GROUP BY region LIMIT 1000",
...
def _exit_audit(container: Any) -> Any:
    return container.audit.record(
        kernel.AuditEvent(
            ...
            decision=kernel.Decision.ESCALATED,
            severity=kernel.Severity.HIGH,
            redacted_summary="Active customers by segment: 2 row(s)",
        )
    )


def _exit_review(container: Any) -> Any:
    return container.review_router.route(_CANONICAL_ANSWER, maker=ACTOR, tenant=TENANT)
...
def _exit_certification(container: Any) -> Any:
    return container.certification.dataset_status("orders_daily")


def _exit_dictionary(container: Any) -> Any:
    return container.dictionary.lookup("total revenue by region")


def _exit_llm(container: Any) -> Any:
    return container.llm.propose_intent("total revenue by region", ())


def _exit_query_engine(container: Any) -> Any:
    return container.query_engine.execute(_CANONICAL_QUERY)


def _exit_guardrail(container: Any) -> Any:
    return container.guardrail.screen("total revenue by region")


def _exit_channel(container: Any) -> Any:
    return container.channel.deliver(_CANONICAL_ANSWER)
...
#:
#: IDENTITY is the load-bearing one for the exposure guard: what the bound identity adapter
#: DECLARES is the single flag the guard reads before it stands down and lets the process bind
#: every interface, so a portability tour that toured every seam except that one would skip the
#: seam whose exit behaviour matters most.
EXIT_CALLS: dict[str, Callable[[Any], Any]] = {
    ...
    "certification": _exit_certification,
    "dictionary": _exit_dictionary,
    "llm": _exit_llm,
    "query_engine": _exit_query_engine,
    "guardrail": _exit_guardrail,
    "channel": _exit_channel,
}
