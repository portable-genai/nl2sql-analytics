...
| `rename_fork.py` | Not a demo script: the one-pass mechanical rebrand a FORK runs once, rewriting the package name, the console-script name, the `NL2SQL` env prefix, the Terraform resource stem and the distribution id. It lives here because it is stdlib-only and offline like everything else in this directory, and it is listed because `tests/unit/test_demo_surface.py` requires every `scripts/*.py` to appear in this table. It prints a plan and writes nothing without `--yes`. See [`../docs/ADOPTING.md`](../docs/ADOPTING.md). |
