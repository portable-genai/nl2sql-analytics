...
Rewrites the python package name, the console-script name, the ``NL2SQL`` env-var prefix, the
...
the certified semantic layer, row caps, fixtures, eval golden set) that this script deliberately
...
# --------------------------------------------------------------------------------------- #
_OLD_PACKAGE = "nl2sql_analytics"
_OLD_CLI = "nl2sql_analytics"
_OLD_ENV_STEM = "NL2SQL"
_OLD_RESOURCE = "h1-svc"
_OLD_DIST = "nl2sql-analytics"
...
# File suffixes considered text (everything else is skipped). ``.jsonl`` is here because this
# repo's eval golden set is JSON Lines and its header comment names the service.
_TEXT_SUFFIXES = {
    ".py",
    ".toml",
    ".yaml",
...
def _rules(args: argparse.Namespace) -> list[tuple[str, str, str]]:
    ...
    return [
        ...
        # The env-var prefix. Matched WITHOUT the trailing underscore so both NL2SQL_PROFILE
...
def main() -> int:
    ...
    parser.add_argument("--cli", required=True, help="new console-script name, e.g. acme-analyst")
    parser.add_argument("--env-prefix", required=True, help="new env-var prefix stem, e.g. ACME")
    parser.add_argument(
        "--resource", required=True, help="new Terraform name_prefix stem, e.g. acme-analyst"
    )
    parser.add_argument(
    ...
    print("Then work through docs/ADOPTING.md: region, IdP, the semantic layer, fixtures, eval.")
    return 0
