# SPEC: NL2SQL Semantic Analyst (H1)
...
- **Maker-checker (P-06) and routing (R8)**: a HIGH/CRITICAL result sets
  ...
  logged rather than failing the result. `NL2SQL_REVIEW_ROUTING` switches routing (default on).
- **Profile**: resolved ONCE, at import, into a `ProfileChoice` and never a bare string. Three
  states of `NL2SQL_PROFILE`: UNSET is NO CHOICE (the SDK-free adapters
