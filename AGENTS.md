...
NL2SQL Semantic Analyst (H1): Governed NL-to-SQL analyst: certified semantic layer, deterministic SQL, cited answers.
Rendered from `hex-service-template`, so it starts at reference parity rather than converging
toward it. Package `nl2sql_analytics`, environment prefix
`NL2SQL`, region `asia-southeast1`.
...
- `src/nl2sql_analytics/domain/` is PURE stdlib. `kernel.py` holds the vertical-neutral
...
- **The exposure guard is derived from the IDENTITY BINDING, and from nothing else.** An end-user
  ...
  `NL2SQL_S2S_TOKEN` may never enter that decision: it authenticates a
...
- **The one adapter that declares `VERIFIED` must EARN it, and it is the one adapter that may not
  ...
  configured `NL2SQL_IAP_AUDIENCE`, three-state: unset or emptied REFUSES,
...
- **One profile read, three states, refused at import.** `config.PROFILE_CHOICE` resolves once at
  ...
  `NL2SQL_PROFILE` (mentioning it in a refusal message is wanted, reading
